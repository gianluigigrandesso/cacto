...
class Manipulator(gym.Env):
    ...
    def get_end_effector_position(self, state, recompute=True):
        ...
        RF = self.robot.model.getFrameId(self.end_effector_frame_id) 
    ...
    def simulate_and_derivative_tf(self,state,u, BATCH_SIZE, der=1):
    ...
    def simulate_tf(self,x,u):
        ...
        t_next  = x[:,-1]+ self.dt
        if self.NORMALIZE_INPUTS:
            q1_next = q1_next / self.state_norm_arr[0]
            q2_next = q2_next / self.state_norm_arr[1]
            q3_next = q3_next / self.state_norm_arr[2]
            v1_next = v1_next / self.state_norm_arr[3]
            v2_next = v2_next / self.state_norm_arr[4]
...
class DoubleIntegrator(gym.Env):
    ...
    def reward_tf(self,x2,u,BATCH_SIZE,last_ts):
        ...
        #for i in range(BATCH_SIZE):
        #    if last_ts[i][0] == 1.0:
        #        vel_joint_list.append(x2_not_norm[i,3]**2 + x2_not_norm[i,4]**2 - 10000/w_v)
        #    else:    
        #        vel_joint_list.append(0)
        #vel_joint = tf.cast(tf.stack(vel_joint_list),tf.float32)
    
        r = (w_d*(-(x_ee[:]-TARGET_STATE[0])**2 -(y_ee[:]-TARGET_STATE[1])**2) + w_peak*peak_reward - w_ob1*ell1_pen - w_ob2*ell2_pen - w_ob3*ell3_pen - w_u*(u[:,0]**2 + u[:,1]**2))/100 
        r = tf.reshape(r, [r.shape[0], 1])
    ...
    def simulate_and_derivative_tf(self,state,u,BATCH_SIZE, der=1):
        ...
        if self.NORMALIZE_INPUTS:
            q0_next = q0_next / self.state_norm_arr[0]
            q1_next = q1_next / self.state_norm_arr[1]
            v0_next = v0_next / self.state_norm_arr[2]
            v1_next = v1_next / self.state_norm_arr[3]
            t_next = 2*t_next/self.state_norm_arr[-1] -1     
    ...
    def simulate_tf(self,x,u):
        x_next  = x[:,0] + self.dt*x[:,2] + 0.5*u[:,0]*self.dt**2
        y_next  = x[:,1] + self.dt*x[:,3] + 0.5*u[:,1]*self.dt**2
        vx_next = x[:,2] + self.dt*u[:,0]
        vy_next = x[:,3] + self.dt*u[:,1]
        t_next  = x[:,-1]+ self.dt
        if self.NORMALIZE_INPUTS:
            x_next = x_next   / self.state_norm_arr[0]
            y_next = y_next   / self.state_norm_arr[1]
            vx_next = vx_next / self.state_norm_arr[2]
