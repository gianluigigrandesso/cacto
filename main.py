...
from Plot_functions import plot_results, plot_policy, plot_policy_eval, rollout, plot_Return, plot_AvgReturn
...
# Save config file
f=open(conf.Config_path+'/Manipulator_config{}.txt'.format(conf.N_try), 'w')
...
cacto_instance = CACTO(env, conf.NORMALIZE_INPUTS, conf.EPISODE_CRITIC_PRETRAINING, conf.tau_upper_bound, 
                conf.tau_lower_bound, conf.dt, system_param, TO_method, system, conf.nsteps_TD_N, conf.soft_max_param, 
                conf.obs_param, conf.weight, conf.TARGET_STATE, conf.NSTEPS, conf.EPISODE_ICS_INIT, conf.TD_N, 
                conf.prioritized_replay_eps, conf.prioritized_replay_alpha, conf.state_norm_arr, 
                conf.UPDATE_LOOPS, conf.UPDATE_RATE, conf.LR_SCHEDULE, conf.update_step_counter, 
                conf.NH1, conf.NH2, conf.wreg_l1_A, conf.wreg_l2_A, conf.wreg_l1_C, conf.wreg_l2_C, 
                conf.boundaries_schedule_LR_C, conf.values_schedule_LR_C, conf.boundaries_schedule_LR_A, 
...
if TO_method == 'pyomo':
    TrOp = TO_Pyomo(env, conf.NORMALIZE_INPUTS, conf.EPISODE_CRITIC_PRETRAINING, conf.tau_upper_bound, 
                conf.tau_lower_bound, conf.dt, system_param, TO_method, system, conf.nsteps_TD_N, conf.soft_max_param, 
                conf.obs_param, conf.weight, conf.TARGET_STATE, conf.NSTEPS, conf.EPISODE_ICS_INIT, conf.TD_N, 
                conf.prioritized_replay_eps, conf.prioritized_replay_alpha, conf.state_norm_arr, 
                conf.UPDATE_LOOPS, conf.UPDATE_RATE, conf.LR_SCHEDULE, conf.update_step_counter, 
    ...
    TrOp = TO_Casadi(env, conf.NORMALIZE_INPUTS, conf.EPISODE_CRITIC_PRETRAINING, conf.tau_upper_bound, 
                conf.tau_lower_bound, conf.dt, system_param, TO_method, system, conf.nsteps_TD_N, conf.soft_max_param, 
                conf.obs_param, conf.weight, conf.TARGET_STATE, conf.NSTEPS, conf.EPISODE_ICS_INIT, conf.TD_N, 
                conf.prioritized_replay_eps, conf.prioritized_replay_alpha, conf.state_norm_arr, 
                conf.UPDATE_LOOPS, conf.UPDATE_RATE, conf.LR_SCHEDULE, conf.update_step_counter, 
...
RLAC = RL_AC(env, conf.NORMALIZE_INPUTS, conf.EPISODE_CRITIC_PRETRAINING, conf.tau_upper_bound, 
                conf.tau_lower_bound, conf.dt, system_param, TO_method, system, conf.nsteps_TD_N, conf.soft_max_param, 
                conf.obs_param, conf.weight, conf.TARGET_STATE, conf.NSTEPS, conf.EPISODE_ICS_INIT, conf.TD_N, 
                conf.prioritized_replay_eps, conf.prioritized_replay_alpha, conf.state_norm_arr, 
                conf.UPDATE_LOOPS, conf.UPDATE_RATE, conf.LR_SCHEDULE, conf.update_step_counter, 
...
CACTO.actor_model.save_weights(conf.NNs_path+"/Manipulator_actor_final.h5")
CACTO.critic_model.save_weights(conf.NNs_path+"/Manipulator_critic_final.h5")
CACTO.target_critic.save_weights(conf.NNs_path+"/Manipulator_target_critic_final.h5")
