...
# State parameters 
x_min = np.array([-np.inf, -np.inf, -np.inf, -np.inf, 0])                  #not used
x_init_min = np.array([-15, -15, -6, -6, 0]) 
x_max = np.array([np.inf, np.inf, np.inf, np.inf, np.inf])            #not used
x_init_max = np.array([ 15,  15,  6,  6, (NSTEPS-1)*dt])             #not used
nb_state = 2 + 2 + 1
...
tau_lower_bound = -2                        # Action upper bound
