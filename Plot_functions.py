...
#import double_integrator_conf as conf # sys_dep #
#import os
#import tensorflow as tf
...
def plot_policy(tau,x,y,steps,n_updates, diff_loc=0, PRETRAIN=0):
    ...
    col = ['ro', 'bo', 'go']
    ax2 = fig.add_subplot(2, 2, conf.robot.na)
...
def plot_policy_eval(x_list,y_list,len_all,n_updates, diff_loc=0, PRETRAIN=0):
...
def rollout(update_step_cntr, actor_model, env, rand_time, N_try, init_states_sim, diff_loc=0, PRETRAIN=0):
    ...
    tau_all_sim = np.empty((len(init_states_sim)*(conf.NSTEPS-1),conf.robot.na))
    ...
    for k in range(len(init_states_sim)):
        ...
        for i in range(conf.NSTEPS-1):
            ...
            tau_all_sim[i + k*(conf.NSTEPS-1), :] = ctrl_sim
            ...
            if i==conf.NSTEPS-2:
                ...
                #tau_arr_sim.append(ctrl_sim)
        if k==2:
            plot_policy(tau_all_sim[(conf.NSTEPS):2*(conf.NSTEPS)],x_ee_arr_sim,y_ee_arr_sim,conf.NSTEPS,update_step_cntr, diff_loc=diff_loc, PRETRAIN=PRETRAIN)
        ...
        len_all = len(init_states_sim) + conf.NSTEPS

    plot_policy_eval(x_ee_all_sim,y_ee_all_sim,len_all,update_step_cntr, diff_loc=diff_loc, PRETRAIN=PRETRAIN)
